...
async function run(): Promise<void> {
  try {
    ...
    const specCount = result.findings.filter(
      (f: Finding) => f.ruleId.startsWith('skill-spec')
    ).length;
    const securityCount = result.findings.filter(
      (f: Finding) => f.ruleId.startsWith('skill-security')
    ).length;
