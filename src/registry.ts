...
export class SkillsRegistryManager {
   ...
   */
  async loadRegistry(): Promise<SkillRegistry | null> {
    const registryPath = this.getRegistryPath();
    if (fs.existsSync(registryPath)) {
      try {
        const content = fs.readFileSync(registryPath, 'utf-8');
        this.registry = JSON.parse(content) as SkillRegistry;
   ...
   */
  async fetchFromSkillsSh(
    limit = SKILLS_SH_API.defaultLimit,
    maxTotal = SKILLS_SH_API.maxTotalSkills,
    verbose = false
  ...
  async updateRegistry(
    ...
    const skills = await this.fetchFromSkillsSh(
      SKILLS_SH_API.defaultLimit,
      SKILLS_SH_API.maxTotalSkills,
      verbose
    );
   ...
   */
  async getRegistry(forceUpdate = false, verbose = false): Promise<SkillRegistry> {
    if (!forceUpdate) {
...
export class RegistryDuplicateDetector {
  private registryManager: SkillsRegistryManager;
   ...
   */
  private jaccardSimilarity(str1: string | undefined | null, str2: string | undefined | null): number {
  ...
  async checkForDuplicates(
    ...
    for (const registrySkill of registry.skills) {
      ...
      const descSimilarity = this.jaccardSimilarity(
        skillDescription,
...
export async function checkDuplicatesAgainstRegistry(
  skillOrName: string | { name: string; description: string; instructions?: string },
  skillDescription?: string,
  skillContent?: string
): Promise<DuplicateCheckResult> {
  ...
  if (typeof skillOrName === 'object') {
    return detector.checkForDuplicates(
      skillOrName.name,
      skillOrName.description,
      skillOrName.instructions
    );
  }
...
export async function updateSkillsRegistry(verbose = false): Promise<RegistryUpdateStats> {
...
export async function listRegistrySkills(): Promise<SkillMetadata[]> {
  const manager = new SkillsRegistryManager();
