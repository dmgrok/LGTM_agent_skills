...
export class GitleaksDetector implements SecretDetector {
  name = 'gitleaks';
  ...
  async detect(content: string, _filePath?: string): Promise<SecurityFinding[]> {
...
export class TruffleHogDetector implements SecretDetector {
  name = 'trufflehog';
...
export class FallbackSecretDetector implements SecretDetector {
...
export class SecurityScanner implements QualityEvaluator {
  ...
  private async initializeSecretDetector(): Promise<void> {
    ...
    const detectors: Record<string, SecretDetector> = {
  ...
  async scan(skill: RawSkill): Promise<SecurityScanResult> {
    const startTime = Date.now();
    const findings: SecurityFinding[] = [];
    const secretFindings: SecurityFinding[] = [];
