...
export class SkillTestRunner {
     ...
     */
    evaluateAssertions(output, assertions) {
        const errors = [];
     ...
     */
    async runTestCase(testCase, skillContent, agentRunner) {
     ...
     */
    async runTests(skillName, skillContent, testCases, agentRunner) {
        const startTime = Date.now();
        const results = [];
        let passed = 0;
...
export class SkillScaffolder {
     ...
     */
    scaffold(options) {
        const { name, description = 'TODO - Describe what this skill does and when to use it.', version = '1.0.0', availableSkills = [], includeTests = true, includeScripts = false, includeReferences = false } = options;
